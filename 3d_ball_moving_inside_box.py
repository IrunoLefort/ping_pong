...
paddle_x_position = 0
paddle_y_position = 0
paddle_z_position = room_z/2 - wall_thick/2
paddle_opacity = 0.5
...
while(True):
    ...
    paddle_y_position = (-room_y/1023.0)*y + room_y/2
    ...
    if(paddle_y_position >= room_y/2+paddle_x/2):
        paddle_y_position = room_y/2+paddle_x/2
    if(paddle_y_position <= -room_y/2+paddle_x/2):
        paddle_y_position = -room_y/2+paddle_x/2
    ...
    if y_test1 or y_test2:
        delta_y = delta_y*(-1)
    
    ball_z_position = ball_z_position + delta_z
    z_test1 = ball_z_position+ball_radius>room_z/2-wall_thick/2
    z_test2 = ball_z_position-ball_radius<-room_z/2+wall_thick/2
    if z_test2:
        delta_z = delta_z*(-1)
    
    test_paddle_x = paddle_x_position-paddle_x/2<ball_x_position and ball_x_position<paddle_x_position+paddle_x/2
    test_paddle_y = paddle_y_position-paddle_y/2<ball_y_position and ball_y_position<paddle_y_position+paddle_y/2
    if z_test1:
        if test_paddle_x and test_paddle_y:
            delta_z = delta_z*(-1)
        else:
            lab = label(text="GAME OVER!", color=color.red)
            time.sleep(2)
            os._exit()
    my_ball.pos = vector(ball_x_position, ball_y_position, ball_z_position)
    my_paddle.pos = vector(paddle_x_position, paddle_y_position, paddle_z_position)